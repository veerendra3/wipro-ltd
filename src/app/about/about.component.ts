...
export class AboutComponent implements OnInit {
  ...
  //Array of objects representing a movie class
  movie:Movie[]=[
  ...
  getColor(){ // === string equality comparision operator, which returns false 
                  //for the values which are not of the similar type 
    return this.serverStatus === 'Online' ? 'green' : 'red';
  }

  getColor1(country:string){
    switch (country) {
      case 'UK':
        return 'green';
      case 'USA':
